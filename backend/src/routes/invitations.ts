  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (
        ...
        companyCheck.rows[0].owner_id !== ownerId
      ) {
        return res
          .status(403)
          .json({
            error:
              "You are not authorized to send invitations for this company.",
          });
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (status === "accepted") {
        ...
        const practitionerCheck = await pool.query(
          "SELECT user_id FROM practitioners WHERE user_id = $1",
          [userId]
        );
        if (practitionerCheck.rows.length === 0) {
          ...
          return res
            ...
            .json({
              error:
                "You must create a practitioner profile before accepting an invitation.",
            });
        }
